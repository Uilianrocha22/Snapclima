...
const apiKey = "e316270620dcaf48399883b66fd91723";
...
navigator.geolocation.getCurrentPosition(
  ...
  (err) => {
    if (err.code === 1) {
      alert(
        "Geolocalização negada pelo usuario ,busque manualmente por uma cidade através da barra de pesquisa. "
      );
    } else {
      console.log(err);
    }
...
function getCityWeather(cityName) {
  weatherIcon.src = `imagens/loading-icon.svg`;
...
function displayWeather(data) {
  ...
  weatherIcon.src = `imagens/${icon}.svg`;
  weatherDescription.textContent = description;
  currentTemperature.textContent = `${Math.round(temp)}°C`;
  windSpeed.textContent = `${Math.round(speed * 3.6)} km/h`;
  thermalSensation.textContent = `${Math.round(feels_like)}°C`;
